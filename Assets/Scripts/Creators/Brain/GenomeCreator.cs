...
public class GenomeCreator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        foreach (Transform child in page_button_panel.transform)
        {
            page_buttons.Add(child.GetComponent<Button>());
        }
    ...
    void CreateGUINodesFromCPPN(Transform page_transform, CPPNGenome genome)
    {
        ...
        for (int to_idx = 0; to_idx < genome.CPPN_nodes.Length; to_idx++)
        {
            ...
            for (int j= start_idx; j < end_idx; j++)
            {
                ...
                //setup link
                link.Initialize();
                link.SetTargetsAndPosition(pos1.localPosition, pos2.localPosition);
            }
        }
